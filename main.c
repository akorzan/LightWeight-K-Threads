...
void *
fn_bounce(void *d, lwt_chan_t chan) 
{
	int i;
...
void *
fn_null(void *d, lwt_chan_t chan)
{ return NULL; }
...
void *
fn_identity(void *d, lwt_chan_t chan)
{ return d; }

void *
fn_nested_joins(void *d, lwt_chan_t chan)
{
	lwt_t chld;
...
void *
fn_sequence(void *d, lwt_chan_t chan)
{
...
void *
fn_join(void *d, lwt_chan_t chan)
{
	lwt_t t = (lwt_t)d;
	void *r;
...
void *
fn_chan(void *data, lwt_chan_t chan)
{
	lwt_chan_t from;
	lwt_chan_t to = data;
	int i;
...
test_perf_channels(int chsz)
{
	...
	assert(LWT_RUNNABLE == lwt_current()->state);
	from = lwt_chan(chsz);
	assert(from);
	//lwt_chan_grant(from);
...
void *
fn_snder(void *arg, lwt_chan_t chan)
{
...
void *
fn_async_steam(void *data, lwt_chan_t chan)
{
	lwt_chan_t to = data;
	int i;
...
void *
fn_grpwait(void *d, lwt_chan_t chan)
{
	lwt_chan_t c = d;
	int i;
...
test_grpwait(int chsz, int grpsz)
{
	...
	assert(lwt_cgrp_free(g)); /* Changed to allow 1 to pass assert */
	/**
...
void *
fn_test_create_channel(void *d, lwt_chan_t chan)
{
	lwt_snd(chan, (void *)0xDEADBEEF);
	lwt_die(NULL);
}

void *
fn_test_kthread(void* d, lwt_chan_t chan)
{

	lwt_t k_child1;
	k_child1 = lwt_create(fn_test_create_channel, NULL, LWT_NO_JOIN, chan);
	/* the master thread will be blocked */
	printf("Before: Runnable: %d, Blocked: %d, Dead: %d\n",
	       lwt_info(LWT_INFO_NTHD_RUNNABLE),
	       lwt_info(LWT_INFO_NTHD_BLOCKED),
	       lwt_info(LWT_INFO_NTHD_ZOMBIES));
	test_perf();
	printf("After: Runnable: %d, Blocked: %d, Dead: %d\n",
	       lwt_info(LWT_INFO_NTHD_RUNNABLE),
	       lwt_info(LWT_INFO_NTHD_BLOCKED),
	       lwt_info(LWT_INFO_NTHD_ZOMBIES));
#if 1	
...
void
test_create_channels(void)
{
	lwt_t chld1;
	lwt_chan_t chan;

	chan = lwt_chan(0);
	lwt_kthd_create(fn_test_kthread, NULL, chan);
	assert(0xDEADBEEF == (unsigned int) lwt_rcv(chan));
	lwt_chan_deref(chan);
}
